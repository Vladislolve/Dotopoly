...
/*function PlayerBoughtEstate(data)
{
		var player;
		var text = "";
		var price = "";
		...
		text = data.text;
		price = data.price;
		player = Game.GetPlayerInfo( data.pID );
		hero = player.player_name;
		$.GetContextPanel().SetHasClass( "leader_has_been_killed", true );
		$( "#KillMessage_Text" ).text = text;
		$( "#KillMessage_Hero" ).text = hero;
		$( "#KillMessage_Bounty").text = price;
		Game.EmitSound("Tutorial.TaskProgress");
		$.Schedule( 5, ClearKillMessage );
} */


function PlayerWarning(data)
{
...
(function () {
    ...
    // GameEvents.Subscribe( "estate_bought", PlayerBoughtEstate);
})();
